...
$countQuery = "
    ...
    FROM Promoters p
    WHERE p.ParentPromoterID = :promoterId
";
...
$countStmt->bindParam(':promoterId', $promoterId);
...
$totalChildPromoters = $totalPromoters; // Use the count from the count query
...
$statsStmt->bindParam(':promoterId', $promoterId);
...
foreach ($allPromoters as $promoter) {
    ...
    $totalCustomers += $promoter['CustomerCount'];
...
<head>
    ...
    <title>Child Promoters | Golden Dreams</title>
        ...
        <div class="main-content">
            <div class="section-header">
                <div class="section-title">
                    ...
                    <div class="section-info">
                        <h2>Child Promoters</h2>
                        <p>View and manage your team of promoters</p>
                    </div>
                </div>
            </div>
            ...
            <?php if (empty($childPromoters)): ?>
                <div class="empty-state">
                    ...
                    <p>You don't have any child promoters yet. When you add promoters to your team, they will appear here.</p>
                </div>
            <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($childPromoters as $promoter): ?>
                            <tr>
                                ...
                                <td>
                                    <div class="stat-value"><?php echo $promoter['PaymentCount']; ?></div>
                                </td>
                                <td>
                                    <div class="stat-value">₹<?php echo number_format($promoter['TotalVerifiedAmount'], 2); ?></div>
                                </td>
                                <td>
                                    <div class="promoter-actions">
                                        <a href="view.php?id=<?php echo $promoter['PromoterID']; ?>" class="action-btn" title="View Details">
                ...
                <?php if ($totalPages > 1): ?>
                    <div class="pagination">
                        <?php if ($page > 1): ?>
                            <a href="?page=1<?php echo !empty($statusFilter) ? '&status=' . urlencode($statusFilter) : ''; ?><?php echo !empty($searchQuery) ? '&search=' . urlencode($searchQuery) : ''; ?>" class="pagination-item">
                                <i class="fas fa-angle-double-left"></i>
                            </a>
                            <a href="?page=<?php echo $page - 1; ?><?php echo !empty($statusFilter) ? '&status=' . urlencode($statusFilter) : ''; ?><?php echo !empty($searchQuery) ? '&search=' . urlencode($searchQuery) : ''; ?>" class="pagination-item">
                        ...
                        if ($startPage > 1) {
                            echo '<a href="?page=1' . (!empty($statusFilter) ? '&status=' . urlencode($statusFilter) : '') . (!empty($searchQuery) ? '&search=' . urlencode($searchQuery) : '') . '" class="pagination-item">1</a>';
                            if ($startPage > 2) {
                        ...
                        for ($i = $startPage; $i <= $endPage; $i++) {
                            if ($i == $page) {
                                ...
                                echo '<a href="?page=' . $i . (!empty($statusFilter) ? '&status=' . urlencode($statusFilter) : '') . (!empty($searchQuery) ? '&search=' . urlencode($searchQuery) : '') . '" class="pagination-item">' . $i . '</a>';
                            }
                        }
                        ...
                        if ($endPage < $totalPages) {
                            ...
                            echo '<a href="?page=' . $totalPages . (!empty($statusFilter) ? '&status=' . urlencode($statusFilter) : '') . (!empty($searchQuery) ? '&search=' . urlencode($searchQuery) : '') . '" class="pagination-item">' . $totalPages . '</a>';
                        }
                        ?>
                        ...
                        <?php if ($page < $totalPages): ?>
                            ...
                            <a href="?page=<?php echo $totalPages; ?><?php echo !empty($statusFilter) ? '&status=' . urlencode($statusFilter) : ''; ?><?php echo !empty($searchQuery) ? '&search=' . urlencode($searchQuery) : ''; ?>" class="pagination-item">
