...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (
            empty($customerSub['BankName']) || empty($customerSub['BankAccountName']) ||
            empty($customerSub['BankAccountNumber']) || empty($customerSub['IFSCCode'])
        ) {
            throw new Exception('Please update your bank details before subscribing');
        }
        ...
        // Move uploaded file
        if (!move_uploaded_file($screenshot['tmp_name'], $uploadPath)) {
        ...
        // Create payment record for current installment
        $stmt = $db->prepare("
