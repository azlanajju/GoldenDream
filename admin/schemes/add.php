    ...
    try {
        ...
        if (isset($_POST['installment'])) {
            foreach ($_POST['installment'] as $index => $installment) {
                ...
                if (!isset($errors["installment_$index"]) && 
        ...
        if (empty($errors)) {
            // Insert scheme
            $stmt = $conn->prepare("
                INSERT INTO Schemes (SchemeName, Description, MonthlyPayment, TotalPayments, Status) 
                VALUES (?, ?, ?, ?, 'Active')
            ");
            $stmt->execute([$schemeName, $description, $monthlyPayment, $totalPayments]);
            ...
            foreach ($installments as $index => $installment) {
                ...
                // Handle image upload
                if (isset($_FILES['installment']['tmp_name'][$index]) && 
