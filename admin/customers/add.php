...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $prefix = 'CUST';
            $stmt = $conn->query("SELECT MAX(CAST(SUBSTRING(CustomerUniqueID, 5) AS UNSIGNED)) as max_id FROM Customers WHERE CustomerUniqueID LIKE '$prefix%'");
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            $nextId = ($result['max_id'] ?? 0) + 1;
            $customerUniqueID = $prefix . str_pad($nextId, 6, '0', STR_PAD_LEFT);
